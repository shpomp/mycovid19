...
public class MyTestsRepo {
  ...
  public static List<Test> fetchAll() throws SQLException {
    String sql = " SELECT * FROM " + table
        + " WHERE (test_status = 'booked' OR test_status = 'done') AND user_id = 30 ORDER BY test_date ";
  ...
  // did not work:
  // String sql = " UPDATE " + table + " SET test_status = 'available', user_id =
  // default WHERE test_id = ? ";
  // String sql = " UPDATE " + table + " SET test_status = 'available', user_id =
  // 'default' WHERE test_id = ? ";
  // String sql = " UPDATE " + table + " SET test_status = 'available', user_id =
  // 'null' WHERE test_id = ? ";
